...
void drawSquare(Canvas& dest, const Eigen::Vector2i& center, int side,
                uint8_t color) {
  cv::rectangle(dest, cv::Point(center[0] - side / 2, center[1] - side / 2),
                cv::Point(center[0] + side / 2, center[1] + side / 2),
                cv::Scalar(color, color, color));
}

void drawSquareFilled(Canvas& dest, const Eigen::Vector2i& center, int side,
        uint8_t color) {
  cv::rectangle(dest, cv::Point(center[0] - side / 2, center[1] - side / 2),
        cv::Point(center[0] + side / 2, center[1] + side / 2),
        cv::Scalar(color, color, color), cv::FILLED);
}
