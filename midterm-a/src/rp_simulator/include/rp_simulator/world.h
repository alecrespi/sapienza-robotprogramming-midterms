...
class World : public WorldItem {
 public:
  ...
  /**
   * @brief Adds a landmark to the world.
   *
   * @param id The ID of the landmark.
   * @param x The x-coordinate of the landmark.
   * @param y The y-coordinate of the landmark.
   */
  void addLandmark(const int id, const float x, const float y) {
    Eigen::Vector2f position(x, y);
    _landmarks.push_back(position);
    _landmark_ids.push_back(id);
  }
   ...
   *
   * Landmarks are drawn as squares on the canvas.
   *
  ...
  void draw(Canvas &canvas) const {
    ...
    for (size_t i = 0; i < _landmarks.size(); ++i) {
      Eigen::Vector2i l_px = _grid_map->worldToGrid(_landmarks[i]);
      drawSquare(canvas, l_px, 5, 90);
    }
  }

  /**
   * @brief Gets the landmark positions.
   *
   * @return The landmark positions.
   */
  const std::vector<Eigen::Vector2f> &landmarks() const { return _landmarks; }

  /**
   * @brief Gets the landmark IDs.
   *
   * @return The landmark IDs.
   */
  const std::vector<int> &landmarkIds() const { return _landmark_ids; }
 ...
 protected:
  ...
  std::vector<Eigen::Vector2f> _landmarks;  // Landmark positions
  std::vector<int> _landmark_ids;           // Landmark IDs
};