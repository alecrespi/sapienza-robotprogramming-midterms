...
int main(int argc, char** argv) {
  ...
  // Create robots, scanners, and landmark scanners
  for (const auto& robot_config : config["robots"]) {
    ...
    // Create landmark scanners
    for (const auto& scanner_config : robot_config["landmark_scanners"]) {
      std::string scanner_name = scanner_config["name"].as<std::string>();
      std::vector<float> scanner_in_robot =
          scanner_config["scanner_in_robot"].as<std::vector<float>>();
      float scanner_radius = scanner_config["scanner_radius"].as<float>();
      ...
      auto scanner = std::make_shared<LandmarkScanner>(
          *robot, scanner_name, node,
          fromCoefficients(scanner_in_robot[0], scanner_in_robot[1],
                           scanner_in_robot[2]),
          scanner_freq);
      scanner->_radius = scanner_radius;
      landmark_scanners.push_back(scanner);
    }
  }

  // Add landmarks to the world
  for (const auto& landmark_config : config["landmarks"]) {
    int landmark_id = landmark_config["id"].as<int>();
    std::vector<float> landmark_position =
        landmark_config["position"].as<std::vector<float>>();

    world_object->addLandmark(landmark_id, landmark_position[0],
                              landmark_position[1]);
  }
  ...
  // Create a canvas to draw the grid map
  Canvas canvas;
